 ...
 * 그 토큰의 로그인 계정 클레임을 읽는다. 인증은 이미 필터에서 끝났으므로 여기서는 예외를 던지지 않는다.
 */
@Component
class CurrentLoginIdentityArgumentResolver(
    private val jwtTokenProvider: JwtTokenProvider,
) : HandlerMethodArgumentResolver {
    override fun supportsParameter(parameter: MethodParameter): Boolean =
        parameter.hasParameterAnnotation(CurrentLoginIdentity::class.java)
    ...
    override fun resolveArgument(
        ...
        return runCatching { jwtTokenProvider.getLoginIdentity(token) }.getOrNull()
    }
}
